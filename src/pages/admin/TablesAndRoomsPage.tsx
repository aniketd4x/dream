...
export default function TablesAndRoomsPage() {
  ...
  const handleDownloadQR = async () => {
    ...
    await downloadImageFile(dataUrl, `Room_${qrModalRoom.room_number}_QR.png`);
  };

  // Print Room Standee Card
  const handlePrintQRStandee = () => {
    ...
    const guestUrl = `${window.location.origin}/room/${qrModalRoom.qr_token}`;

    const standHtml = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; width: 340px; margin: 20px auto; padding: 28px; border: 2px solid #0f172a; border-radius: 24px; text-align: center; box-shadow: 0 10px 25px rgba(0,0,0,0.1); background: #ffffff;">
        <div style="background: linear-gradient(135deg, #0f172a, #1e293b); color: #ffffff; padding: 12px 20px; border-radius: 16px; margin-bottom: 20px;">
          <h1 style="margin: 0; font-size: 18px; font-weight: 800; letter-spacing: -0.5px;">${restaurant?.name || 'Hotel & Resort'}</h1>
          <p style="margin: 3px 0 0; font-size: 11px; color: #94a3b8; text-transform: uppercase; letter-spacing: 1px;">Room Hospitality Hub</p>
        </div>

        <div style="margin-bottom: 16px;">
          <span style="font-size: 11px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 1.5px;">Welcome to</span>
          <h2 style="margin: 2px 0 0; font-size: 32px; font-weight: 900; color: #0f172a; letter-spacing: -1px;">ROOM ${qrModalRoom.room_number}</h2>
          <span style="display: inline-block; background: #e0e7ff; color: #3730a3; font-size: 11px; font-weight: 700; padding: 3px 10px; border-radius: 9999px; margin-top: 4px;">
            ${qrModalRoom.room_type} • Floor ${qrModalRoom.floor_number}
          </span>
        </div>

        <div style="display: inline-block; padding: 16px; background: #f8fafc; border: 2px dashed #cbd5e1; border-radius: 20px; margin-bottom: 18px;">
          <img src="${qrDataUrl}" width="200" height="200" style="display: block; border-radius: 12px;" />
        </div>

        <div style="background: #f1f5f9; padding: 12px 14px; border-radius: 14px; margin-bottom: 16px;">
          <h3 style="margin: 0 0 4px; font-size: 13px; font-weight: 800; color: #0f172a;">Scan with Phone Camera</h3>
          <p style="margin: 0; font-size: 11px; color: #475569; line-height: 1.4;">
            🍽️ Order Room Service Food<br/>
            🧹 Request Housekeeping & Towels<br/>
            💧 Extra Mineral Water & Toiletries<br/>
            🧾 View Live Room Bill & Checkout
          </p>
        </div>
    ...
    `;
    printIframeHtml(standHtml, `Room_${qrModalRoom.room_number}_Standee`);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-28">
      ...
      {qrModalRoom && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-backdrop">
          <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col animate-scale-in">
            {/* Modal Header */}
            <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <QrCode className="w-4 h-4 text-theme-primary" />
                <h3 className="text-sm font-bold text-slate-900">Room {qrModalRoom.room_number} QR Code</h3>
              </div>
              <button
            ...
            <div className="p-6 text-center space-y-4">
              ...
              <div
                ref={qrCanvasRef}
                className="bg-slate-50 border-2 border-slate-200/80 rounded-2xl p-5 shadow-xs flex flex-col items-center"
              >
                <div className="mb-2">
                  <span className="text-[10px] uppercase font-extrabold tracking-widest text-slate-400">
                    {restaurant?.name || 'Hotel'}
                  </span>
                  <h2 className="text-2xl font-black text-slate-900">ROOM {qrModalRoom.room_number}</h2>
                </div>

                <div className="p-3 bg-white rounded-xl shadow-xs border border-slate-200 mb-2">
                  <QRCodeCanvas
                    value={`${window.location.origin}/room/${qrModalRoom.qr_token}`}
                    size={170}
                    level="H"
                    includeMargin={false}
                  />
                ...
                <p className="text-[11px] font-bold text-slate-600">Scan for Hospitality & Food</p>
                <span className="text-[10px] text-slate-400 font-mono mt-0.5">{qrModalRoom.qr_token}</span>
              </div>

              {/* Copy URL Bar */}
              <div className="flex items-center gap-1.5 p-2 rounded-xl bg-slate-100 border border-slate-200 text-left">
                <span className="text-[11px] text-slate-600 font-mono truncate flex-1 pl-1">
                  {`${window.location.origin}/room/${qrModalRoom.qr_token}`}
                </span>
                <button
                  type="button"
                  onClick={async () => {
                    await copyTextToClipboard(`${window.location.origin}/room/${qrModalRoom.qr_token}`);
                    setCopiedUrl(true);
                    triggerHaptic('light');
                    setTimeout(() => setCopiedUrl(false), 2000);
                  }}
                  className="px-2.5 py-1 rounded-lg bg-white shadow-xs text-xs font-bold text-slate-800 hover:bg-slate-50 shrink-0"
                >
              ...
              <div className="grid grid-cols-2 gap-2.5 pt-1">
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold transition native-press"
                >
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl bg-theme-gradient text-white text-xs font-bold shadow-theme hover:opacity-95 transition native-press"
                >
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                ...
                <a
                  href={`/room/${qrModalRoom.qr_token}`}
                  target="_blank"
                  rel="noreferrer"
